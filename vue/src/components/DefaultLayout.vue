...
import { Bars3Icon, XMarkIcon, HomeIcon, Bars3BottomLeftIcon,
Bars3BottomRightIcon, UserGroupIcon } from '@heroicons/vue/24/outline'
  ...
  <section class="flex md:flex-row flex-col">
    ...
    <div :class="[isOpen ? 'w-72' : 'md:w-16', 'duration-500 bg-green-700 min-h-screen px-2 md:block hidden']">
      ...
      <nav class="mt-4 flex flex-col gap-4 relative">
        <div @click="logout" class="flex gap-2 px-3 items-center py-2  bg-green-900 rounded-md" v-if="isOpen">
    ...
    <div class="bg-green-700 h-16 w-full md:hidden px-4 relative duration-500">
      ...
      <!-- overlay -->
      <div :class="[isOpen ? 'absolute top-0 left-0 w-full bg-black opacity-50 min-h-screen' : 'hidden', 'duration-500']"
        @click="isOpen = !isOpen"></div>
      <!-- modal menu -->
      <div
        :class="[isOpen ? 'w-72' : 'w-0 overflow-hidden', 'absolute top-0 left-0 bg-green-700 min-h-screen duration-500']">
        <!-- menu items desktop -->
        ...
        <nav class="mt-8 flex flex-col gap-2 relative px-4 duration-500">
          <div @click="logout" class="flex gap-4 px-3 items-center py-2  bg-green-900 rounded-md whitespace-pre"
            v-if="isOpen">
            <img :src="user.imageUrl" class="h-10 w-10 rounded-full" alt="">
            <div class="flex flex-col justify-start text-sm font-medium text-white w-full ">
              <a @click="logout" class="cursor-pointer py-1 px-1 hover:bg-green-600 rounded-md w-fit ">Sign out </a>
